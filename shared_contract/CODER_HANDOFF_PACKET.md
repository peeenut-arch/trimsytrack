...
Repo (origin): https://github.com/peeenut-arch/BACKENDTRIMSY.git
...
This backend is shared across multiple clients (mobile apps + PC). Clients MUST identify which app is calling the backend using `app_id`.
...
- There are no profiles and no profile onboarding.
...
- `receiptCreate` (purchase receipts) is **TrimsyApp-only** (`app_id=trimsyapp`). TrimsyPC must not create purchase receipts.

Required inputs:

- All write calls require `clientProtocolVersion` + `app_id`.
- `receiptCreate` requires:
	- `idempotencyKey`
	- `drivingTripId` (must exist and be owned by the caller)
	- `vendorName`
	- `currency`
...
- All write-like calls send `app_id` and the backend rejects missing/invalid app IDs.
...
Hard rule:
- If an endpoint is not listed here, it is not part of the deployed backend surface.
...
- There are no profiles. Clients must not implement profile selection/switching.
...
- `receiptRowCreate`
- `receiptMediaSet`
- `receiptMediaListByReceipt`
- `receiptSnapshotGet`
- `productCostAllocate`

### Driving journal truth

- `drivingTripCreate`

Trip↔receipt linking:
- No explicit backend link endpoint. TrimsyApp derives the association from trip data it reads and stores it on `receiptCreate` (`drivingTripId`).

### Finalize snapshots

- `finalizeLogSnapshotCreate`
...
- `finalizeLogSnapshotGetCallable`
- `finalizeLogSnapshotsListByProductCallable`

- `receiptCreateCallable`
- `receiptRowCreateCallable`
- `receiptMediaSetCallable`
- `receiptMediaListByReceiptCallable`
- `receiptSnapshotGetCallable`
- `productCostAllocateCallable`

- `drivingTripCreateCallable`
